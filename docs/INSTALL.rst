...
Using TLS with Apache (Recommended):
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
2. Get TLS certificate for your server. You can use [Let's Encrypt](https://letsencrypt.org/)
to generate free certificates. [Certbot](https://certbot.eff.org/) makes this process easier.
Note that the certificates are only valid for 3 months, but you can set it up to auto-renew.

3. Create a file ``/etc/httpd/conf.d/wsgi.conf``, with the contents::
...
Most of this file was generated by Certbot. I have only added the wsgi bits.

4. Create another configuration file ``/etc/httpd/conf.d/redirect.conf``, with the contents::
  ...
  RewriteEngine On
  RewriteCond %{HTTP_HOST} !^your-server-name.com$
  RewriteRule /.* https://your-server-name.com/ [R]

  ErrorLog /var/log/httpd/redirect.error.log
  LogLevel warn
  </VirtualHost>

This will redirect any requests coming to http://IPADDR or https://IPADDR or http://SERVER.com
to https://SERVER.com
